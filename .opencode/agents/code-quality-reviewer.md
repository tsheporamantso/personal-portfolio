...
This is a Create React App (react-scripts 5) + React 18 single-page application using:

- JavaScript/JSX (no TypeScript)
- State/data-fetching split three ways — match the pattern the section already uses:
  - Services: Redux Toolkit slice
...
- Jest + React Testing Library for testing (CRA) — `vitest` is unused
- React Router v6 (`react-router-dom`), with `/` single-page portfolio, `/login`, `/tip`, `/dashboard`
- Styling via SCSS/plain CSS (stylelint), no Tailwind
- All backend endpoints centralized in `src/utils/api.js` (`REACT_APP_API_URL` + `/api/v1/*`)

Key coding standards to enforce:

- NO semicolons in JavaScript
- Minimal dependencies philosophy (eslint hints: re-export to avoid object spreading props into DOM components)
- Prefer functional components and hooks; avoid class components
- Use React Router's navigation (Link/useNavigate) instead of raw `<a>` tags where appropriate
- Ensure accessible markup (proper button vs link semantics, labels, alt text)
